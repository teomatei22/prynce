...
exp_cutoff = 3 * 1.0e2  # cutoff to avoid overflow warnings
epsrel_low = 1.0e-1  # minimum precision sufficient to speed up some quad integrals
if PRyMini.compute_nTOp_thermal_flag:
...
def FermiCoulomb(b):
    ...
    Gamma = np.sqrt(1.0 - PRyMini.alphaem**2.0) - 1.0
    gamma1 = 1.0 + Gamma
    gamma2 = 3.0 + 2.0 * Gamma
    Fn_Compton = PRyMini.hbar * PRyMini.clight / me
    return (
        (1.0 + Gamma / 2.0)
        * 4.0
        * ((2.0 * PRyMini.radproton * b) / Fn_Compton) ** (2.0 * Gamma)
        / (gamma(gamma2) ** 2)
        * np.exp((np.pi * PRyMini.alphaem) / b)
        / ((1.0 - b**2) ** Gamma)
        * np.abs(gamma(gamma1 + (PRyMini.alphaem / b) * 1j)) ** 2
    )


def RadCorrResum(b, y, en):
    # Additional constants specific to radiative corrections in [Czarnecki et al. 2004]
    mA = 1.2 * 1.0e3  # MeV , Eq.(9) in hep-ph/0406324
    ...
    Sirlin_fun = (
        3.0 * np.log(mp / (me))
        - 3.0 / 4.0
        + 4.0 * (Rd - 1.0) * (y / (3.0 * en) - 3.0 / 2.0 + np.log(2.0 * y))
        + Rd * (2.0 * (1.0 + b**2) + y**2 / (6.0 * en**2) - 4.0 * b * Rd)
        - (4.0 / b) * spence(1.0 - (2 * b) / (1.0 + b))
    )
    ...
    return (
        (
            ...
            + PRyMini.alphaem
            / (2.0 * np.pi)
            * (Sirlin_fun - 3.0 * np.log(mp / (2 * Q)))
        )
        * (
            Lndecay
            + (PRyMini.alphaem / np.pi) * Cndecay
            + PRyMini.alphaem / (2 * np.pi) * deltandecay * 2 * np.pi / PRyMini.alphaem
        )
        * (
            Sndecay
            + 1.0 / (134.0 * 2.0 * np.pi) * (np.log(mp / mA) + Agndecay)
            + NLLndecay
        )
    )
...
def ComputeFn():
    ...
    def Fn_Born_int(E):
        if (-1.0 >= E) or (E >= 1):
            return E * (E - (Q / me)) ** 2 * np.sqrt(E**2 - 1.0)
        else:
            return 0.0

    Fn_Born = quad(Fn_Born_int, 1.0, Q / me)[0]
    ...
    def Fn_rad_int(e):
        ...
        return (
            ...
            * b
            * FermiCoulomb(b)
            * RadCorrResum(np.sqrt(e**2 - 1.0) / e, q - e, e)
        )

    Fn_rad = quad(Fn_rad_int, 1.0, Q / me)[0]

    # Finite mass corrections to Born approximation
    def ChiFMnDec(en, pe):
        mnOme = mn / me
        f1n = ((1.0 + PRyMini.gA) ** 2.0 + 2.0 * PRyMini.deltakappa * PRyMini.gA) / (
            1.0 + 3.0 * PRyMini.gA**2
        )
        f2n = ((1.0 - PRyMini.gA) ** 2.0 - 2.0 * PRyMini.deltakappa * PRyMini.gA) / (
            1.0 + 3.0 * PRyMini.gA**2
        )
        f3n = (PRyMini.gA**2 - 1.0) / (1.0 + 3.0 * PRyMini.gA**2)
        return (
            f1n * (en - Q / me) ** 2 * (pe**2 / (mnOme * en))
            - f2n / mnOme * (en - Q / me) ** 3
            + (f1n + f2n + f3n)
            / (2.0 * mnOme)
            * (4.0 * (en - Q / me) ** 3 + 2 * (en - Q / me) * pe**2)
    ...
    def Fn_FM_int(pe):
        return (
            pe**2
            * ChiFMnDec(np.sqrt(pe**2 + 1.0), pe)
            * RadCorrResum(
                pe / np.sqrt(pe**2 + 1.0),
                np.abs(np.sqrt(pe**2 + 1.0) - Q / me),
                np.sqrt(pe**2 + 1.0),
            )
            * FermiCoulomb(pe / np.sqrt(pe**2 + 1.0))
        )

    Fn_FM = quad(Fn_FM_int, 0.0, np.sqrt((Q / me) ** 2 - 1.0))[0]
...
def ComputeWeakRates(Tvec):
    ...
    def FD_nu_e4p2(E, phi, x):
        if (
            (2.0 * phi < exp_cutoff)
            and (E * x + phi < exp_cutoff)
            and (2.0 * E * x < exp_cutoff)
        ):
            return (
                E**2
                * np.exp(phi)
                * (
                    (24.0 - E * x * (E * x + 8.0)) * np.exp(E * x + phi)
                    + np.exp(2 * E * x) * (E * x - 6.0) * (E * x - 2.0)
                    + 12 * np.exp(2 * phi)
                )
    ...
    def FD_nu_e2p2(E, phi, x):
        if (
            (3.0 * phi < exp_cutoff)
            and (2 * E * x + phi < exp_cutoff)
            and (E * x < exp_cutoff)
        ):
            return (
                (E * x * (E * x - 4.0) + 2.0) * np.exp(2 * E * x + phi)
                + (4.0 - E * x * (E * x + 4.0)) * np.exp(E * x + 2 * phi)
                + 2 * np.exp(3 * phi)
            ) / (np.exp(E * x) + np.exp(phi)) ** 3
        else:
            return 0.0
    ...
    def FD_nu_e4p1(E, phi, x):
        if (phi < exp_cutoff) and (E * x < exp_cutoff):
            return (
                np.exp(phi) * E**3 * (4 * np.exp(phi) + np.exp(E * x) * (4.0 - E * x))
    ...
    def FD_nu_e2p1(E, phi, x):
        if (phi < exp_cutoff) and (E * x < exp_cutoff):
            return (
                np.exp(phi) * E * (2 * np.exp(phi) + np.exp(E * x) * (2.0 - E * x))
            ) / (np.exp(E * x) + np.exp(phi)) ** 2
        else:
            return 0.0
    ...
    def FD_nu_e3p1(E, phi, x):
        if (phi < exp_cutoff) and (E * x < exp_cutoff):
            return (
                np.exp(phi) * E**2 * (3 * np.exp(phi) + np.exp(E * x) * (3.0 - E * x))
            ) / (np.exp(E * x) + np.exp(phi)) ** 2
        else:
            return 0.0
    ...
    def FD_nu_e3p2(E, phi, x):
        if (
            ...
            return (
                ...
                * np.exp(phi)
                * (
                    (12.0 - E * x * (E * x + 6.0)) * np.exp(E * x + phi)
                    + np.exp(2.0 * E * x) * (E * x * (E * x - 6.0) + 6.0)
                    + 6 * np.exp(2.0 * phi)
                )
            ) / (np.exp(E * x) + np.exp(phi)) ** 3
        else:
    ...
    def D_FD2(E, x):
        if (x * E) < exp_cutoff:
            return -x * np.exp(x * E) / (1.0 + np.exp(x * E)) ** 2
        else:
            return 0.0
    ...
    def ChiFunc(E, p, x, znu, sgnq):
        return (
            FD_nu3(E - sgnq * (Q / me), sgnq * xi_nu, znu)
            * FD2(-E, x)
            * (E - sgnq * (Q / me)) ** 2
        )
    ...
    def ChiFunc_FM(en, pe, x, znu, sgnq):
        ...
        f_1 = (
            (1.0 + sgnq * PRyMini.gA) ** 2.0
            + 2.0 * PRyMini.deltakappa * sgnq * PRyMini.gA
        ) / (1.0 + 3.0 * PRyMini.gA**2)
        f_2 = (
            (1.0 - sgnq * PRyMini.gA) ** 2.0
            - 2.0 * PRyMini.deltakappa * sgnq * PRyMini.gA
        ) / (1.0 + 3.0 * PRyMini.gA**2)
        f_3 = (PRyMini.gA**2 - 1.0) / (1.0 + 3.0 * PRyMini.gA**2)
        FD2_en = FD2(-en, x)
        return (
            f_1 * FD_nu_e2p0(enu(en, sgnq), 0, znu) * FD2_en * (pe**2 / (M_sgnq * en))
            + f_2 * FD_nu_e3p0(enu(en, sgnq), 0, znu) * FD2_en * (-(1.0 / M_sgnq))
            + (f_1 + f_2 + f_3)
            / (2.0 * x * M_sgnq)
            * (
                FD_nu_e4p2(enu(en, sgnq), 0, znu) * FD2_en
                + FD_nu_e2p2(enu(en, sgnq), 0, znu) * FD2_en * pe**2
            )
            + (f_1 + f_2 + f_3)
            / (2.0 * M_sgnq)
            * (
                FD_nu_e4p1(enu(en, sgnq), 0, znu) * FD2_en
                + FD_nu_e2p1(enu(en, sgnq), 0, znu) * FD2_en * pe**2
            )
            - (f_1 + f_2)
            / (x * M_sgnq)
            * (
                FD_nu_e3p1(enu(en, sgnq), 0, znu) * FD2_en
                + FD_nu_e2p1(enu(en, sgnq), 0, znu) * FD2_en * pe**2 / (-en)
            )
            - f_3 * 3.0 / (x * M_sgnq) * FD_nu_e2p0(enu(en, sgnq), 0, znu) * FD2_en
            + f_3
            / (3 * M_sgnq)
            * FD_nu_e3p1(enu(en, sgnq), 0, znu)
            * FD2_en
            * pe**2
            / en
            + f_3
            ...
            * 3.0
            / (2.0 * x)
            * (1.0 - (Mn / Mp) ** sgnq)
            * (FD_nu_e2p1(enu(en, sgnq), 0, znu) * FD2_en)
        )
    ...
    def IPENdpFMCCR(p, x, znu, sgnq):
        ...
        return p**2 * (
            ...
            * RadCorrResum(en_ratio, np.abs(sgnq * Q / me - eOFpe), eOFpe)
            * FermiStat(sgnq, 1, en_ratio)
            + ChiFunc_FM(-eOFpe, p, x, znu, sgnq)
            * RadCorrResum(en_ratio, np.abs(sgnq * Q / me + eOFpe), eOFpe)
            * FermiStat(sgnq, -1, en_ratio)
        )
    ...
    def IPENdpFrom_Chi_CCR(E, p, x, znu, sgnq):
        return p**2 * (
            ...
            * RadCorrResum(p / E, np.abs(sgnq * Q / me - E), E)
            * FermiStat(sgnq, 1, p / E)
            + ChiFunc(-E, p, x, znu, sgnq)
            * RadCorrResum(p / E, np.abs(sgnq * Q / me + E), E)
            * FermiStat(sgnq, -1, p / E)
        )
    ...
    if PRyMini.compute_nTOp_thermal_flag:
        ...
        def A(E, k):
            pE = np.sqrt(E**2 - 1.0)
            return (2.0 * E**2 + k**2) * (np.log((E + pE) / (E - pE))) - 4.0 * pE * E

        def B(E):
            pE = np.sqrt(E**2 - 1.0)
            return 2.0 * E * (np.log((E + pE) / (E - pE))) - 4.0 * pE
        ...
        def IPENCCRT(E, k, x, znu, sgnq):
            ...
            def BE(EkBT):
                ...
                resvec[my_index[:]] = 1.0 / (np.exp(argvec[my_index[:]]) - 1.0)
                return resvec
            ...
            def FD2(en, xval):
                ...
                resvec[my_index_overflow[:]] = 1.0 / (
                    np.exp(np.sign(argvec[my_index_overflow[:]]) * exp_cutoff) + 1.0
                )
                return resvec
            ...
            return (
                ...
                * (
                    ...
                    * (
                        FD2(-E, x)
                        * FermiStat(sgnq, 1, pE / E)
                        * (
                            Chitilde(E - k, znu, sgnq)
                            + Chitilde(E + k, znu, sgnq)
                            - 2 * Chitilde(E, znu, sgnq)
                        )
                        + FD2(E, x)
                        * FermiStat(sgnq, -1, pE / E)
                        * (
                    ...
                    * (
                        FD2(-E, x)
                        * FermiStat(sgnq, 1, pE / E)
                        * (Chitilde(E - k, znu, sgnq) - Chitilde(E + k, znu, sgnq))
                        + FD2(E, x)
                        * FermiStat(sgnq, -1, pE / E)
                        * (Chitilde(-E + k, znu, sgnq) - Chitilde(-E - k, znu, sgnq))
                    )
        ...
        def IPENCCRDiffBremsstrahlung(E, k, x, znu, sgnq):
            q = Q / me
            pE = np.sqrt(E**2 - 1.0)
            Fp = (2.0 * E**2 + k**2) * (np.log((E + pE) / (E - pE))) - 4.0 * pE * E
            Fp += k * (2.0 * E * (np.log((E + pE) / (E - pE))) - 4.0 * pE)
            Fm = (2.0 * E**2 + k**2) * (np.log((E + pE) / (E - pE))) - 4.0 * pE * E
            Fm -= k * (2.0 * E * (np.log((E + pE) / (E - pE))) - 4.0 * pE)
            ...
            res_fac = PRyMini.alphaem / (2.0 * np.pi * k)
            ...
            res1vec[my_index[:]] -= (
                Fp[my_index[:]]
                * FD2(E[my_index[:]] - sgnq * q, znu)
                * (np.abs(E[my_index[:]] - sgnq * q) - k[my_index[:]]) ** 2
            )
            res1vec[:] *= res1_fac[:]
        ...
        def C1dE(E, x, znu, sgnq):
            pE = np.sqrt(E**2 - 1.0)
            return (
                -((PRyMini.alphaem * E) / (2.0 * np.pi * pE))
                * (2.0 * np.pi**2)
                / (3.0 * x**2)
        ...
        def C2dE1dE2(e1v, e2v, x, znu, sgnq):
            ...
            e1pe2 = e1v + e2v
            e1me2 = e1v - e2v
            min_e1pe2 = 2.0 + np.abs(e1me2)
            max_e1pe2 = 2.0 + max(10.0, 15.0 / x) + np.abs(e1me2)
            index_limits = np.where(
                ((e1pe2 - min_e1pe2) > 0) * ((max_e1pe2 - e1pe2) > 0)
            )[0]
            ...
            def D_FD2(en, xval):
                ...
                resvec[my_index[:]] = (
                    -xval
                    * np.exp(argvec[my_index[:]])
                    / (np.exp(argvec[my_index[:]]) + 1.0) ** 2
                )
                return resvec
            ...
            e1 = e1v[index_limits[:]]
            e2 = e2v[index_limits[:]]
            p1 = np.sqrt(e1v[index_limits[:]] ** 2 - 1.0)
            p2 = np.sqrt(e2v[index_limits[:]] ** 2 - 1.0)
            L_fac = np.log((e1 * e2 + p1 * p2 + 1.0) / (e1 * e2 - p1 * p2 + 1.0))
            resvec_limits = (
                PRyMini.alphaem
                / (2.0 * np.pi)
                * (ChiFunc(e1, p1, x, znu, sgnq) + ChiFunc(-e1, p1, x, znu, sgnq))
                * (
                    -(1.0 / 4.0)
                    * np.log(((p1 + p2) / (p1 - p2)) ** 2)
                    * np.log(((p1 + p2) / (p1 - p2)) ** 2)
                    * (
                        D_FD2(e2, x) * p2 / p1 * e1**2 / e2 * (e1 + e2)
                        + FD2(e2, x) * e1**2 / (p1 * p2) * (e2 + e1 / e2**2)
                    )
                    + np.log(((p1 + p2) / (p1 - p2)) ** 2)
                    * (
                        D_FD2(e2, x)
                        * (
                            p2**2 * e1 / e2 * (1.0 / p1**2 + 2.0)
                            - e1**2 * p2 / p1 * L_fac
                        )
                        + FD2(e2, x)
                        * (
                            e1 / (p1**2 * e2**2) * (e2**2 + 2 * p1**2 + 1.0)
                            - (e1**2 + e2**2) / (e1 + e2)
                            - (e1**2 * e2) / (p1 * p2) * L_fac
                        )
                    )
                    - FD2(e2, x) * (4.0 * e1 * p2 / p1 + 2.0 * e2 * L_fac)
                )
            )
        ...
        def L_nTOpThermalTruePhoton(T):
            x = me / (PRyMini.kB * T)
            min_E = 1.001
            max_E = max(10.0, 20.0 / x)
            min_k = 0.001
            max_k = max(10.0, 20.0 / (me / (PRyMini.kB * T)))
        ...
        def L_nTOpThermalDiffBremsstrahlung(T):
            min_E = 1.001
            max_E = max(10.0, 20.0 / (me / (PRyMini.kB * T)))
            min_k = 0.001
            max_k = max(10.0, 20.0 / (me / (PRyMini.kB * T)))
            integ = vegas.Integrator([[min_E, max_E], [min_k, max_k]])
        ...
        def L_nTOpThermal_1_int(E, T):
            return C1dE(
                E, me / (PRyMini.kB * T), me / (PRyMini.kB * T * T_nuOverT(T)), 1
            )
        ...
        def L_nTOpThermal_1(T):
            return quad(
                L_nTOpThermal_1_int,
                1.0,
                max(25.0, 150.0 * (PRyMini.kB * T) / me),
                args=(T),
                epsrel=1.0e-2,
            )[0]
        ...
        def L_nTOpThermal_2_3_int(e1pe2, e1me2, T):
            ...
            xnu = me / (PRyMini.kB * T * T_nuOverT(T))
            return 0.5 * C2dE1dE2(
                (e1pe2 + e1me2) / 2.0, (e1pe2 - e1me2) / 2.0, x, xnu, 1
            )

        def L_nTOpThermal_2_3(T):
            x = me / (PRyMini.kB * T)
            # res_2
            min_e1me1 = -max(10.0, 15.0 / x)
            max_e1me2 = -0.001
            min_e1pe2 = 2.001 + min(np.abs(min_e1me1), np.abs(max_e1me2))
            max_e1pe2 = 2.0 + max(np.abs(min_e1me1), np.abs(max_e1me2))
        ...
        T_nTOp_thermal_interval = np.logspace(
            np.log10(PRyMini.T_end),
            np.log10(PRyMini.T_start),
            PRyMini.sampling_nTOp_thermal,
        )
        ...
        if PRyMini.save_nTOp_thermal_flag:
            np.savetxt(
                my_dir + "/PRyMrates/nTOp/" + "nTOp_thermal_corrections.txt",
                np.c_[T_nTOp_thermal_interval, L_nTOpCCRTh_res],
            )
            np.savetxt(
                my_dir + "/PRyMrates/nTOp/" + "pTOn_thermal_corrections.txt",
                np.c_[T_nTOp_thermal_interval, L_pTOnCCRTh_res],
            )
        if PRyMini.verbose_flag:
            print("n <--> p thermal corrections computed")
    else:
        T_nTOp_thermal_interval, L_nTOpCCRTh_res = np.loadtxt(
            my_dir + "/PRyMrates/nTOp/" + "nTOp_thermal_corrections.txt", unpack=True
        )
        T_nTOp_thermal_interval, L_pTOnCCRTh_res = np.loadtxt(
            my_dir + "/PRyMrates/nTOp/" + "pTOn_thermal_corrections.txt", unpack=True
        )
    ################################
    # Splining thermal corrections #
    ################################
    L_nTOpCCRTh_interp = interp1d(
    ...
    T_interval_HT = np.logspace(
        np.log10(PRyMini.T_start), np.log10(PRyMini.T_weak), PRyMini.sampling_nTOp
    )
    if PRyMini.verbose_flag:
        print("Re-computing n <--> p weak rates @ mid T regime")
    T_interval_MT = np.logspace(
        np.log10(PRyMini.T_weak), np.log10(PRyMini.T_nucl), PRyMini.sampling_nTOp
    )
    if PRyMini.verbose_flag:
        print("Re-computing n <--> p weak rates @ low T regime")
    T_interval_LT = np.logspace(
        np.log10(PRyMini.T_nucl), np.log10(PRyMini.T_end), PRyMini.sampling_nTOp
    )
    if PRyMini.save_nTOp_flag:
        np.savetxt(
            my_dir + "/PRyMrates/nTOp/" + "nTOp_frwrd_HT.txt",
            np.c_[T_interval_HT, nTOp_frwrd_vec(T_interval_HT)],
        )
        np.savetxt(
            my_dir + "/PRyMrates/nTOp/" + "nTOp_bkwrd_HT.txt",
            np.c_[T_interval_HT, nTOp_bkwrd_vec(T_interval_HT)],
        )
        np.savetxt(
            my_dir + "/PRyMrates/nTOp/" + "nTOp_frwrd_MT.txt",
            np.c_[T_interval_MT, nTOp_frwrd_vec(T_interval_MT)],
        )
        np.savetxt(
            my_dir + "/PRyMrates/nTOp/" + "nTOp_bkwrd_MT.txt",
            np.c_[T_interval_MT, nTOp_bkwrd_vec(T_interval_MT)],
        )
        np.savetxt(
            my_dir + "/PRyMrates/nTOp/" + "nTOp_frwrd_LT.txt",
            np.c_[T_interval_LT, nTOp_frwrd_vec(T_interval_LT)],
        )
        np.savetxt(
            my_dir + "/PRyMrates/nTOp/" + "nTOp_bkwrd_LT.txt",
            np.c_[T_interval_LT, nTOp_bkwrd_vec(T_interval_LT)],
        )
    return [
        T_interval_HT,
        nTOp_frwrd_vec(T_interval_HT),
        nTOp_bkwrd_vec(T_interval_HT),
        T_interval_MT,
        nTOp_frwrd_vec(T_interval_MT),
        nTOp_bkwrd_vec(T_interval_MT),
        T_interval_LT,
        nTOp_frwrd_vec(T_interval_LT),
        nTOp_bkwrd_vec(T_interval_LT),
    ]
